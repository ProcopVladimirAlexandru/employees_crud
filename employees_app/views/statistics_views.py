...
LOGGER_NAME = 'statistics_views_logger'
...
def avg_industry_age(request):
    ...
    try:
        ...
        df['age'] = (datetime.datetime.now() - pd.to_datetime(
            df['date_of_birth'], format='%Y-%m-%d')).apply(lambda d: d.days//365)
...
def interesting_statistics(request):
    ...
    try:
        ...
        df['zodiac_sign'] = pd.to_datetime(df['date_of_birth'], format='%Y-%m-%d').apply(
            lambda d: get_zodiac_sign(d.month, d.day))
        zodiac_sign_tups = sorted([(k, v) for k, v in df.groupby(
            ['zodiac_sign'])['salary'].median().to_dict().items()],key=lambda tup: tup[1], reverse=True)
